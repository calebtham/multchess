...
function updateChat() {
  ...
  chatDisplay.innerHTML = game.chat
    .map(
      (message) => `
        <p class="${
          message.playerNumber == me.number ? "me" : "opponent"
        }">${htmlEntities(message.content)}</p>
    `
    )
    .join("");
...
function updateButtons() {
  ...
  if (
    ((me.colour == Game.Piece.white && game.board.history) ||
      (me.colour == Game.Piece.black &&
        game.board.history &&
        game.board.history.history)) &&
  ...
  if (
    game.board.isGameFinished &&
    !me.rematchRequestRecieved &&
    !me.opponentDisconnected
  ) {
...
function drawTakenPieces(ctx) {
  ...
  if (player.colour == Game.Piece.white) {
    ...
    if (game.board.whiteAdvantage > 0) {
      // If white has advantage
...
function drawBoardColour() {
  ...
  for (let i = 0; i < 8; i++) {
    for (let j = 0; j < 8; j++) {
      ...
      if (
        game.board.movedFrom == Game.convert2dTo1d(i, j) ||
        game.board.movedTo == Game.convert2dTo1d(i, j)
      ) {
        colourSquare(
          offset + multiplier * i,
          offset + multiplier * j,
...
function drawBoardPieces() {
  for (let i = 0; i < 64; i++) {
    if (i != game.board.hiddenSquare) {
      drawPiece(
        Game.convert1dTo2d(i).x,
        Game.convert1dTo2d(i).y,
        game.board.square[i]
      );
    }
  }
