...
function handleGameCodeKeyPress(e) {
  if (e.key == "Enter") {
    ...
    // If there was an error before, disregard it
...
function handleClick(e) {
  if (!game.board.isGameFinished) {
    ...
    let boardIndex = Game.convert2dTo1d(
      offset + mouse.x * multiplier,
      offset + mouse.y * multiplier
    );
    ...
    // Dropping a piece
    if (
      game.board.inHand ^ Game.Piece.none && // Check there is a piece in hand with bitwise XOR
      (!Game.isPieceColour(
        game.board.square[boardIndex],
        game.board.colourToMove
      ) ||
        game.board.isLegalMove[boardIndex])
    ) {
      let start = game.board.movedFrom;
      let captured =
        (game.board.square[boardIndex] != 0 &&
          !Game.isSameColour(
            game.board.square[boardIndex],
            game.board.square[start]
          )) ||
        (game.board.enPassantSquare == boardIndex &&
          Game.isPieceType(game.board.square[start], Game.Piece.pawn));
      ...
      if (!madeMove) {
        ...
        let check =
          (game.board.whiteInCheck && opponent.colour == Game.Piece.white) ||
          (game.board.blackInCheck && opponent.colour == Game.Piece.black);
      ...
      if (
        ...
        // Check there is a piece in hand with bitwise XOR
        game.board.isLegalMove = new Array(64).fill(false);
        game.board.hiddenSquare = -1;
        game.board.inHand = Game.Piece.none;
...
function handleHover(e) {
  if (!isTouchDevice()) {
    ...
    if (game.board.inHand ^ Game.Piece.none) {
      // Check there is a piece in hand with bitwise XOR
      if (
        Game.isPieceColour(game.board.square[boardIndex], me.colour) &&
        !game.board.isLegalMove[boardIndex]
      ) {
