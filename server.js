...
io.on("connection", (client) => {
  ...
  function handleMoveMade(nextBoard) {
    ...
    if (roomName && nextBoard) {
      ...
      if (initBoard.colourToMove == client.player.colour) {
        // Check client was meant to make the next move
  ...
  function handleRematchAccept() {
    ...
    if (
      ...
      let startingPlayer =
        state[roomName].game.board.startingPlayer == 1 ? 2 : 1; // switch starting player
      ...
      state[roomName][1].timeLeft =
        (prevGame.timer ? prevGame.timer : Infinity) * 60;
      state[roomName][2].timeLeft =
        (prevGame.timer ? prevGame.timer : Infinity) * 60;
  ...
  function handleTakebackAccept() {
    ...
    if (
      ...
      state[roomName].game.serviceTakeback(
        state[roomName][client.player.number].colour
      );
      emitState(roomName);
    }
  }
